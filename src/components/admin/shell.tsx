  ...
  const sidebar = (
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between px-5 py-5">
        ...
        <button onClick={() => setOpen(false)} className="rounded-lg p-1 text-white/70 hover:text-white lg:hidden" aria-label="Cerrar menú"><X className="h-5 w-5" /></button>
      </div>
      <nav className="flex-1 space-y-5 overflow-y-auto px-3 pb-6 scrollbar-thin" aria-label="Administración">
        {NAV.map((g) => (
          <div key={g.group}>
            <p className="mb-1.5 px-3 text-[10px] font-bold uppercase tracking-[0.18em] text-white/60">{g.group}</p>
            {g.items.map((it) => (
              <Link key={it.href} href={it.href} onClick={() => setOpen(false)} aria-current={active(it.href) ? "page" : undefined} className={cn("flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm font-medium text-white/80 transition hover:bg-white/5 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-glow", active(it.href) && "bg-brand text-white shadow-[0_8px_20px_-10px_rgba(20,184,166,.8)] hover:bg-brand")}>
                <it.icon className="h-4 w-4 shrink-0" />
                <span className="flex-1 truncate">{it.label}</span>
                {badges[it.href] > 0 && <span className="rounded-full bg-accent px-1.5 text-[10px] font-bold text-white" aria-label={`${badges[it.href]} pendientes`}>{badges[it.href]}</span>}
              </Link>
            ))}
          </div>
        ))}
    ...
    <div className="min-h-screen bg-bg lg:grid lg:grid-cols-[264px_1fr]">
      ...
      {open && (
        <div className="fixed inset-0 z-50 lg:hidden">
          <div className="absolute inset-0 bg-ink/60" onClick={() => setOpen(false)} aria-hidden />
          <div ref={drawerRef} role="dialog" aria-modal="true" aria-label="Menú de administración" tabIndex={-1} className="absolute inset-y-0 left-0 w-72 bg-admin shadow-float outline-none">{sidebar}</div>
        </div>
      )}
      <div className="flex min-h-screen flex-col">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-4 border-b border-line bg-bg/85 px-4 backdrop-blur-xl sm:px-6">
          <div className="flex items-center gap-3">
            <button onClick={() => setOpen(true)} className="rounded-lg p-2 hover:bg-muted lg:hidden" aria-label="Abrir menú" aria-expanded={open} aria-haspopup="dialog"><Menu className="h-5 w-5" /></button>
            <Link href="/" target="_blank" className="btn-outline py-1.5 text-xs"><ExternalLink className="h-3.5 w-3.5" /> Ver sitio</Link>
          </div>
          <div className="flex items-center gap-2">
            <Link href="/admin/consultas" className="relative rounded-full p-2 hover:bg-muted" aria-label={pending.inquiries > 0 ? `${pending.inquiries} consultas nuevas` : "Consultas"} title="Consultas nuevas">
              <MessageSquare className="h-5 w-5 text-ink-soft" />
              {pending.inquiries > 0 && <span className="absolute -right-0.5 -top-0.5 h-2.5 w-2.5 rounded-full bg-accent ring-2 ring-bg" aria-hidden />}
            </Link>
            <Dropdown
              ...
              trigger={
                <button className="flex items-center gap-2 rounded-full border border-line bg-elevated py-1 pl-1 pr-2.5 hover:border-brand focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/50">
                  <Avatar src={session?.user?.image} name={session?.user?.name ?? "A"} size="sm" />
                  <span className="hidden text-sm font-semibold sm:block">{session?.user?.name}</span>
                  <ChevronDown className="h-4 w-4 text-ink-muted" aria-hidden />
                </button>
              }
            >
              <DropdownItem href="/cuenta/perfil">Mi perfil</DropdownItem>
              <DropdownItem href="/cuenta">Mi cuenta</DropdownItem>
              <DropdownItem onClick={() => signOut({ callbackUrl: "/" })} className="text-danger"><LogOut className="h-4 w-4" /> Salir</DropdownItem>
            </Dropdown>
          </div>
        </header>
