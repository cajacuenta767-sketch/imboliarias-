...
export function Price({ amount, currencyCode = "USD", period, type, className, compact }: { amount: number; currencyCode?: string; period?: string | null; type?: string; className?: string; compact?: boolean }) {
  ...
  return (
    <span className={cn("font-display font-extrabold tabular-nums", className)}>
      {price(amount, currencyCode, { compact })}
