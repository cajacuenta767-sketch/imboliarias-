...
import { Bold, Italic, Strikethrough, List, ListOrdered, Heading2, Heading3, Link2, Quote, Undo, Redo, Minus } from "lucide-react";
import { cn } from "@/lib/utils";

export function RichEditor({ value, onChange, minHeight = 220 }: { value: string; onChange: (html: string) => void; minHeight?: number }) {
  ...
  const B = ({ on, active, children, title }: { on: () => void; active?: boolean; children: React.ReactNode; title: string }) => (
    <button type="button" title={title} aria-label={title} aria-pressed={active} onMouseDown={(e) => { e.preventDefault(); on(); }} className={cn("rounded-lg p-1.5 text-ink-soft hover:bg-muted hover:text-ink focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/50", active && "bg-brand-soft text-brand-strong")}>
      {children}
    </button>
  );
  return (
    <div className="overflow-hidden rounded-xl border border-line bg-elevated focus-within:border-brand focus-within:ring-2 focus-within:ring-brand/20">
      <div className="flex flex-wrap items-center gap-0.5 border-b border-line bg-muted/60 px-2 py-1.5" role="toolbar" aria-label="Formato">
        ...
        <B title="Tachado" on={() => editor.chain().focus().toggleStrike().run()} active={editor.isActive("strike")}><Strikethrough className="h-4 w-4" /></B>
        <span className="mx-1 h-5 w-px bg-line" />
