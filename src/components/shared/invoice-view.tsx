...
type Inv = { id: string; number: string; status: string; createdAt: string; paidAt?: string | null; subtotal: number; discount: number; tax: number; total: number; currencyCode: string; package?: { name: string; credits: number; durationDays: number } | null; user: { name: string; email: string; phone?: string | null }; payments: { gateway: string; reference?: string | null; status: string; createdAt: string }[]; coupon?: { code: string } | null };
...
export function InvoiceView({ invoice: inv, settings: s, backHref = "/cuenta/facturas" }: { invoice: Inv; settings: Record<string, string>; backHref?: string }) {
  const money = (n: number) => `${inv.currencyCode} ${formatNumber(n, 2)}`;
  return (
    <div>
      ...
      <div className="card mx-auto max-w-3xl p-8 print:border-0 print:shadow-none sm:p-10">
        ...
        <div className="ml-auto mt-4 max-w-xs space-y-1 text-sm">
          ...
          {inv.tax > 0 && <div className="flex justify-between"><span className="text-ink-soft">Impuestos ({s.tax_percent}%)</span><span>{money(inv.tax)}</span></div>}
